...
let shieldPosX = [
	[[100, 100, 100],[120, 120,],[140, 140],[160,160,160]],
	[[400, 400, 400],[420, 420],[440, 440],[460, 460, 460]],
	[[700, 700, 700],[720, 720],[740, 740],[760,760, 760]],
	[[1000, 1000, 1000],[1020, 1020],[1040, 1040],[1060, 1060, 1060]]
];
let shieldPosY = [
	...
	for (let block = 0; block < shieldPosX.length; block++) {
		for (let collumns = 0; collumns < shieldPosX[block].length; collumns++) {
			for (let piece = 0; piece < shieldPosX[block][collumns].length; piece++) {
				for (let bulletsIndex = 0; bulletsIndex < bulletsX.length; bulletsIndex++){
					if (bulletsX[bulletsIndex] >= shieldPosX[block][collumns][piece] && bulletsX[bulletsIndex] <= shieldPosX[block][collumns][piece] + 20 && shieldHealth[block][collumns][piece] > 0) {
						if (bulletsY[bulletsIndex] <= shieldPosY[block][collumns][piece] && bulletsY[bulletsIndex] >= shieldPosY[block][collumns][piece] - 20) {
